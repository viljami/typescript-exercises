...
const sort = function <T>(key: keyof T, dir:number = 1) {
...
const rowToString = function <T>(row: T, state:string): string {
    return `${state}${JSON.stringify(row)}`;
}

const getId = function <T>(row: Partial<T>): Partial<T> {
    const keys = ['_id'] as (keyof T)[];
    return pluck<Partial<T>>(keys)(row);
}
...
export class Database<T> {
    ...
    async delete(query: Query<T>): Promise<number> {
        const [results, fileRows]: [Partial<T>[], string[]] = await Promise.all([
            this.find(query),
            this.getFile()
        ]);
        ...
        try {
            ...
            const removeds = fileRows
                .map(s => idsToRemove.some(id => s.includes(`"_id":${id}`)));

            return fs.writeFile(
                this.filename,
                fileRows
                    .map((s, index) => removeds[index] ? `D${omitFirstLetter(s)}` : s)
                    .join('\n')
            )
        } catch (e) {
            throw e;
    ...
    async insert(row: T) {
        try {
            return fs.appendFile(this.filename, `${rowToString(row, 'E')}\n`)
        } catch(e) {
            throw e;
        }
    }
    ...
    async find(query: Query<T>, options: FindOptions<T> = {}): Promise<Partial<T>[]> {
        ...
        if ('sort' in options) {
            ...
            results = results.sort(sort<T>(keys[0], options.sort[keys[0]]))
        }
